...
### Fixed
- Editing a container no longer silently strips its labels. The edit sheet's prefill omitted labels entirely, so the recreate that applies an edit dropped every label on the container, including Orchard's own sandbox markers (a sandbox stopped being recognised as one after any edit). Labels now carry through: internal ones untouched, the rest editable in the new labels editor.
