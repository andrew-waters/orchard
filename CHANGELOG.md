...
### Added
- A Builds menu on the Images tab tracks every image build started this session: a badge shows how many are running, and each row can reopen the live (or finished) build log in its own sheet or cancel an in-flight build, with durations and a Clear Finished action. Builds now run concurrently and survive closing the Build Image sheet. The list is limited to builds Orchard itself started: apple/container's builder shim exposes only info() and a build stream over its gRPC surface, so BuildKit's build history is not reachable from the host.
